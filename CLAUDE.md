...
- `center` is **`[longitude, latitude]`** (GeoJSON order — easy to flip
  accidentally).
- `id` is the kebab-case slug — unique within the file, stable, and used
  internally as the Supabase Realtime channel key.
- `gid` is a permanent random 10-char alphanumeric id (a-z 0-9, no
  separators). Globally unique across every chat map. Use it for external
  references — hashtags, deep links, third-party services. Never reuse one
  even if the hotspot is deleted; that's the whole point. Generated by
  `tools/add_gid.py` style scripts; do not hand-pick.
